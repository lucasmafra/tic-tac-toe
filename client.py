...
def display_status(status, opponent_id, room):
    ...
    if status == 'opponent_turn':
      print("Waiting for {opponent} to play...".format(opponent = opponent_id))
...
def main():
  ...
  while game_not_finished(game_status):
    if board != last_board or game_status != last_status:
      clear()
      display_board(board)
    ...
    board, game_status, opponent_id = refresh_game_status(sock)
